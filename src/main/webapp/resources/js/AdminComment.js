...
(function () {
  ...
  function getContextPath() {
    ...
    const me = document.currentScript || qs('script[src*="AdminComment.js"]');
    if (me && me.src) {
  ...
  function injectCssOnce() {
    if (qs('#admin-comment-css')) return;
    const style = document.createElement('style');
    style.id = 'admin-comment-css';
    style.textContent = `
      ...
      .comment-card { border:1px solid #eee; border-radius: 12px; padding: 12px; margin-top: 10px; background:#fff; }
      .comment-head { display:flex; gap:10px; }
      .comment-avatar { width:44px; height:44px; border-radius:50%; overflow:hidden; border:1px solid #eee; background:#f2f2f2; flex: 0 0 44px; }
      .comment-avatar img { width:100%; height:100%; object-fit:cover; }
      .comment-meta { flex:1; }
      .comment-date { color:#777; font-size:12px; margin-top:2px; }
      .comment-body { margin-top:10px; white-space:pre-wrap; line-height: 1.4; }
      .comment-actions { margin-top:10px; display:flex; gap:8px; justify-content:flex-end; flex-wrap:wrap; }
      .comment-suspend { margin-top:10px; padding-top:10px; border-top:1px dashed #eee; display:none; }
      ...
      .susp-days { display:flex; gap:8px; flex-wrap:wrap; }
      .radio-pill { display:inline-flex; align-items:center; gap:6px; padding:6px 10px; border:1px solid #ddd; border-radius:999px; cursor:pointer; user-select:none; background:#fff; }
    ...
    root.innerHTML = `
      <section class="admin-section">
        <div class="admin-card">
          <div class="subtabs">
            <button type="button" class="subtab comment-subtab" data-subtab="ALL">전체 댓글</button>
            <button type="button" class="subtab comment-subtab" data-subtab="REPORTED">신고 받은 댓글</button>
          </div>
          ...
          <div class="admin-row" style="margin-bottom:12px; display:flex; gap:10px; flex-wrap:wrap; align-items:end;">
            ...
            <div>
              <div style="font-size:12px;color:#666;margin-bottom:4px;">댓글 내용</div>
            ...
            <div>
              ...
              <select id="c_status" class="admin-select">
                <option value="">전체</option>
            ...
            <button id="btnCommentSearch" type="button" class="admin-btn primary">검색</button>
            <button id="btnCommentReset" type="button" class="admin-btn secondary">초기화</button>
          </div>

          <div class="pager">
            <div>총 <b id="commentTotalCount">0</b>건</div>
            <div style="display:flex; gap:8px; align-items:center;">
              <button id="btnCommentPrev" type="button" class="admin-btn">이전</button>
              <span id="commentPageInfo">1 / 1</span>
              <button id="btnCommentNext" type="button" class="admin-btn">다음</button>
            </div>
          </div>

          <div id="commentError" class="admin-error"></div>
          <div id="commentEmpty" class="admin-empty">조회 결과가 없습니다.</div>
          <div id="commentList"></div>
        </div>
      </section>

      <div id="commentReportBackdrop" class="backdrop">
        <div class="modal">
          <div class="modal-head">
            <b>댓글 신고 내역</b>
            <button id="btnCloseCommentReport" type="button" class="admin-btn">닫기</button>
          </div>
          <div class="modal-body">
            <div id="commentReportList"></div>
          </div>
        </div>
      </div>
    `;
  ...
  // ===== report modal =====
  function openReportBackdrop() { qs('#commentReportBackdrop').classList.add('open'); }
  function closeReportModal() { qs('#commentReportBackdrop').classList.remove('open'); }

  async function openReportModal(commentId) {
    openReportBackdrop();
    const el = qs('#commentReportList');
    el.innerHTML = `<div style="color:#777;">불러오는 중...</div>`;

    const list = await apiGet(`/admin/reports/api/by-target?targetType=COMMENT&targetId=${commentId}`);
  ...
  function renderRows(items) {
    const box = qs('#commentList');
    box.innerHTML = (items || []).map(c => {
      const avatar = c.profileImageUrl ? `<img src="${escapeHtml(c.profileImageUrl)}" alt="avatar" />` : '';
      return `
        <div class="comment-card" data-comment-id="${c.commentId}" data-author="${c.authorAccountId}">
          <div class="comment-head">
            <div class="comment-avatar">${avatar}</div>
            <div class="comment-meta">
              <div>
                <b>#${c.commentId}</b>
                · ${escapeHtml(roleLabel(c.roleType))}
                · <span class="admin-badge">${escapeHtml(statusLabel(c.status))}</span>
              </div>
              <div>${escapeHtml(c.nickname || '')} ${c.handle ? `(${escapeHtml(c.handle)})` : ''}</div>
              <div class="comment-date">${escapeHtml(c.createdAt || '')}</div>
            </div>
          </div>
          ...
          <div class="comment-actions">
            <button type="button" class="admin-btn secondary btnOpenReports" data-id="${c.commentId}">신고 내역</button>
            <button type="button" class="admin-btn secondary btnHideComment" data-id="${c.commentId}">숨김 처리</button>
            <button type="button" class="admin-btn secondary btnToggleSuspend" data-id="${c.commentId}">사용자 정지</button>
          </div>

          <div class="comment-suspend">
			  <div style="display:flex; gap:10px; align-items:flex-end; flex-wrap:wrap;">
			    <div class="admin-field">
			      <label style="display:block; font-size:12px; color:#666; margin-bottom:4px;">정지 일수</label>
			      <select class="admin-select suspendDays" data-id="${c.commentId}">
			    ...
			    <div class="admin-field">
			      <label style="font-size:12px;color:#666;margin-bottom:4px;">사유</label>
			      <input
			        type="text"
			    ...
			    <button type="button"
			      class="admin-btn primary btnSuspendSubmit"
			      data-id="${c.commentId}">
			      정지 실행
			    </button>
			  ...
			  <div style="margin-top:6px;color:#777;font-size:12px;">
			    사유: COMMENT · 정지 성공 시 원인 댓글 자동 숨김
			  </div>
			</div>
        </div>
      `;
  ...
  function highlightFocusedComment(commentId) {
    const card = qs(`.comment-card[data-comment-id="${commentId}"]`);
  ...
  async function handleFocusNavigation() {
    if (state.focusType !== 'COMMENT' || !state.focusTargetId) return;

    const found = highlightFocusedComment(state.focusTargetId);
    if (!found) {
      alert(`대상 댓글(commentId=${state.focusTargetId})이 현재 목록에 없습니다.`);
      clearFocusFromUrl();
      return;
    }
  ...
  function renderMeta(total) {
    ...
    qs('#commentTotalCount').textContent = total || 0;
    qs('#commentPageInfo').textContent = `${state.page} / ${totalPages}`;
    qs('#commentEmpty').style.display = (total === 0) ? 'block' : 'none';
  }

  function showError(e) {
    console.error(e);
    const el = qs('#commentError');
  ...
  async function loadList(pushState) {
    ...
    const data = await apiGet('/admin/comments/api?' + query);
  ...
  function bindEvents() {
    ...
    qsa('.comment-subtab').forEach(btn => {
      btn.addEventListener('click', () => {
        qsa('.comment-subtab').forEach(b => b.classList.toggle('active', b === btn));
    ...
    // search / reset / paging
    qs('#btnCommentSearch').addEventListener('click', () => {
      ...
      state.status = qs('#c_status').value;
    ...
    qs('#btnCommentReset').addEventListener('click', () => {
      state.nickname = '';
      state.handle = '';
      state.content = '';
      state.roleType = '';
    ...
    qs('#btnCommentPrev').addEventListener('click', () => {
      if (state.page <= 1) return;
      state.page--;
      loadList(true).catch(showError);
    });

    qs('#btnCommentNext').addEventListener('click', () => {
    ...
    // modal close
    qs('#btnCloseCommentReport').addEventListener('click', closeReportModal);
    qs('#commentReportBackdrop').addEventListener('click', (e) => {
      if (e.target && e.target.id === 'commentReportBackdrop') closeReportModal();
    });
    ...
    qs('#commentList').addEventListener('click', async (e) => {
      ...
      const hideBtn = t.closest('.btnHideComment');
      if (hideBtn) {
        const id = Number(hideBtn.dataset.id);
        const ok = confirm('이 댓글을 숨김 처리할까요? (status=HIDDEN, deleted=1)');
        if (!ok) return;
        await apiPost(`/admin/comments/api/${id}/hide`);
        alert('숨김 처리 완료');
      ...
      if (toggleBtn) {
        const card = toggleBtn.closest('.comment-card');
        const form = qs('.comment-suspend', card);
      ...
      if (suspendBtn) {
        const card = suspendBtn.closest('.comment-card');
        const commentId = Number(suspendBtn.dataset.id);
        ...
        if (![1,3,7,30].includes(days)) {
          alert('정지 일수는 1/3/7/30 중에서 선택하세요.');
          return;
        }

        const ok = confirm(
          `정지 대상: accountId=${authorId}\n정지: ${days}일\n사유: COMMENT(고정)\n원인 댓글: commentId=${commentId}\n\n진행할까요?`
        );
        if (!ok) return;
  ...
  // ✅ AdminUser.js가 호출할 엔트리 포인트
  async function renderCommentTab() {
    injectCssOnce();
    renderCommentTabHtml();
